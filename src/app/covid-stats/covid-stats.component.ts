...
export class CovidStatsComponent implements OnInit {
  ...
  const totalCasess = document.querySelector(".total_casess");
  const deathss = document.querySelector(".deathss");
  const recoveredd = document.querySelector(".recoveredd");
  const newCasess = document.querySelector(".new_casess");
var settings = {
  async: true,
  crossDomain: true,
  url:
...
function displayData(data) {
  ...
  countries.forEach(function(country) {
    if (country.country_name == "India") {
      ...
      totalCasess.innerHTML = country.cases;
      deathss.innerHTML = country.deaths;
      recoveredd.innerHTML = country.total_recovered;
      newCasess.innerHTML = country.new_cases;
    }
  });
}
